...
public class DefaultEventHandler implements VxEventHandler
{
    ...
    HashMap<Integer, VxCameraManager.CameraPosition> bookmarks = new HashMap<Integer, VxCameraManager.CameraPosition>();
    ...
    **/
                              ...
                              double eye[], double lookat[], double up[])
    {
        ...
        for (int iter = 0; iter < 100; iter++) {
            ...
            double M[][] = VxUtil.lookAt(eye, lookat, up);

            // where does the point project to now?
            double winpos0[] = VxUtil.project(xyz, M, P, viewport);
                                         ...
                                         double dir1[], double dir2[])
    {
        ...
        double M0[][] = VxUtil.lookAt(eye, c, up);
        double w0[] = VxUtil.project(xyz, M0, P, viewport);

        double M1[][] = VxUtil.lookAt(LinAlg.add(eye, LinAlg.scale(dir1, eps)),
                                       LinAlg.add(c, LinAlg.scale(dir1, eps)),
                                       up);
        double w1[] = VxUtil.project(xyz, M1, P, viewport);

        double M2[][] = VxUtil.lookAt(LinAlg.add(eye, LinAlg.scale(dir2, eps)),
                                       LinAlg.add(c, LinAlg.scale(dir2, eps)),
                                       up);
        double w2[] = VxUtil.project(xyz, M2, P, viewport);
    ...
    public boolean mousePressed(VxCanvas vc, VxLayer vl, VxCanvas.RenderInfo rinfo, GRay3D ray, MouseEvent e)
    {
        ...
        if ((mods & InputEvent.BUTTON3_DOWN_MASK) != 0) {
            ...
            VxCameraManager.CameraPosition cameraPosition = rinfo.cameraPositions.get(vl);
    ...
    public boolean keyPressed(VxCanvas vc, VxLayer vl, VxCanvas.RenderInfo rinfo, KeyEvent e)
    {
        ...
        switch (code) {
            ...
            case KeyEvent.VK_F10:
                ...
                VxCameraManager.CameraPosition cameraPosition = bookmarks.get(code);
