...
//            BufferedImage im = ImageIO.read(new File("1_earth_8k.jpg"));
            BufferedImage im = ImageIO.read(new File("/home/ebolson/earth.png"));
...
//        System.out.printf("%d vertices, %d triangles\n", geom.vertices.size(), geom.tris.size());
    ...
    static class MyObject implements VisObject, VisSerializable
    {
        long vid = VisUtil.allocateID(), tid = VisUtil.allocateID(), iid = VisUtil.allocateID();
        ...
        public MyObject(ObjectReader ins)
        {
        }

        public void writeObject(ObjectWriter outs) throws IOException
        {
            outs.writeFloats(verts);
            outs.writeFloats(texcoords);
            outs.writeInts(indices);
            outs.writeObject(tex);
        }

        public void readObject(ObjectReader ins) throws IOException
        {
            verts = ins.readFloats();
            texcoords = ins.readFloats();
            indices = ins.readInts();

            tex = (VisTexture) ins.readObject();
        }
    }
